...
<div>
    ...
    <select v-model="info.disabilities">
        <option disabled value="" >Please select one</option>
       <option v-for ="(disabilitie, index) in disabilities" :key = "index" :value ="disabilitie">{{disabilitie}}</option>
    </select>
...
<!-- need to be a multiselect -->
        <label for="education">Educational Experiences</label>
            ...
            <option disabled value="" >Please Select One</option>
       <option v-for ="(educationalExperience, index) in educationalExperiences" :key = "index" :value ="educationalExperience">{{educationalExperience}}</option>
            </select>
    ...
    export default {
        ...
        computed: {
            years(){
                ...
                const year = new Date();
                return Array.from({ length:  year.getFullYear() -1959}, (_, index) => 1960 + index)
                }
            },
            methods: {
            setPersonalInfo() {
